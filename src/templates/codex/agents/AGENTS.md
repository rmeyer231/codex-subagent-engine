<!-- BEGIN CSE-MANAGED -->
<!--
  Managed block: global Codex native subagent routing policy.
  Edit only by re-running the cse install-codex installer with an explicit
  --apply. Anything outside the BEGIN/END markers belongs to the user and
  is preserved byte-for-byte on every rerun.
-->
...
### Role selection (deterministic)

| Phase                         | Role               | Default sandbox  |
| ----------------------------- | ------------------ | ---------------- |
| Unfamiliar-code exploration   | `cse_explorer`     | `read-only`      |
| PLAN / SPEC drafting          | `cse_planner`      | `read-only`      |
| Approved code changes         | `cse_implementer`  | `workspace-write`|
| Independent verification      | `cse_reviewer`     | `read-only`      |
...
Phase-level model recommendations live in `~/.codex/model-routing.md` and are
referenced from this block. Closer project guidance wins over the global
managed block; the global block wins over the model-routing file when the two
conflict.
